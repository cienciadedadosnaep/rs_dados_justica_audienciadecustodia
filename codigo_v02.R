...
library(readr)
aud_sexo_bio_sexo_bio <- read_csv("data/aud_sexo_bio - sexo_bio.csv")
names(aud_sexo_bio_sexo_bio) = c("ano","Homens","Mulheres")
dados3<- aud_sexo_bio_sexo_bio %>% select(ano,Homens,Mulheres)
nomes <- names(dados3)

dados3 %<>% mutate(`Homens`= round(`Homens`/1000,2))
dados3 %<>% mutate(`Mulheres`= round(`Mulheres`/1000,2))
dados3
...
objeto_0 <- dados3 %>%
  ...
  select(ano,Homens,Mulheres) %>% #filter(ano<2019) %>%
  #arrange(trimestre) %>%
...
titulo<-T_ST_P_No_Justica$TITULO[3]
subtexto<-"Fonte: Defensoria Pública - Bahia"
link <- T_ST_P_No_Justica$LINK[3]
...
write(exportJson0,file = paste('data/',gsub('.csv','',T_ST_P_No_Justica$NOME_ARQUIVO_JS[3]),
                               '.json',sep =''))
write(texto,file = paste('data/',T_ST_P_No_Justica$NOME_ARQUIVO_JS[3],
                         sep =''))
