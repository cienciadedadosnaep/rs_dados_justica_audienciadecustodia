...
library(readr)
dd <- read_delim("data/audiencia_idade.csv", 
                              delim = ";", escape_double = FALSE, trim_ws = TRUE)
names(dd) = c("ano", "18-29","30-41","42-59","60+")
dados<- dd %>% select(ano,`18-29`,`30-41`,`42-59`,`60+`)
nomes <- names(dados)

## Reorganizando a escala

dados %<>% mutate(`18-29`= round(`18-29`/1000,2))
dados %<>% mutate(`30-41`= round(`30-41`/1000,2))
dados %<>% mutate(`42-59`= round(`42-59`/1000,2))
dados %<>% mutate(`60+`= round(`60+`/1000,2))
dados
...
objeto_0 <- dados %>%
  ...
  select(ano,`18-29`,`30-41`,`42-59`,`60+`) %>% #filter(ano<2019) %>%
  #arrange(trimestre) %>%
...
data_serie <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$`18-29`)),
                                   collapse = ' ')),']',sep = '')

data_serie2 <- paste('[',gsub(' ',',',
                              paste(paste(as.vector(objeto_0[[1]]$`30-41`)),
                                    collapse = ' ')),']',sep = '')

data_serie3 <- paste('[',gsub(' ',',',
                              paste(paste(as.vector(objeto_0[[1]]$`42-59`)),
                                    collapse = ' ')),']',sep = '')
